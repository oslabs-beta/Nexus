...
function getImportNodes(programBody) {
  var importNodes = programBody.filter(function (node) {
...
function getVariableNodes(programBody) {
  ...
  var variableNodes = programBody.filter(function (node) {
    return node.type === 'VariableDeclaration';
  });
  return variableNodes;
}
function getNonImportNodes(programBody) {
  var nonImportNodes = programBody.filter(function (node) {
    return node.type !== 'ImportDeclaration';
  });
...
function getChildrenNodes(variableNodes) {
  ...
  var returnNode = nodes.filter(function (node) {
...
function getJsxNodes(childrenNodes) {
  var jsxNodes = childrenNodes.filter(function (node) {
...
function getChildrenComponents(jsxNodes, importNodes) {
  ...
  var importValues = importNodes.map(function (node) {
    return node.source.value;
  });
  var componentPaths = importValues.filter(function (str) {
  ...
  for (var _a = 0, jsxNodes_1 = jsxNodes; _a < jsxNodes_1.length; _a++) {
    ...
    if (firstChar === firstChar.toUpperCase()) {
      ...
      if (cache[''.concat(componentName)]) {
        children = main(cache[''.concat(componentName)]);
      }
